package me.youm.reactor.common.exception;

import lombok.Getter;
import me.youm.reactor.common.enums.EnumInterface;

/**
 * 异常状态码、异常消息 构造器CodeEnum
 *
 * @author youta
 */
@Getter
public class AliOssException extends BaseException implements EnumInterface {

    private static final long serialVersionUID = -1L;
    public AliOssException(){

    }

    public AliOssException(String message) {
        super(message);
    }

    public AliOssException(int code, String message) {
        super(code, message);
    }

    public AliOssException(EnumInterface enums, Object... args) {
        super(enums.getCode(), enums.getMsg(), args);
    }

}
